...
define(["joshlib!uielement","joshlib!utils/dollar","joshlib!vendor/underscore", "joshlib!utils/getscript", "joshlib!vendor/webglearth"], function(UIElement,$,_, getScript, useless) {
  var UIMap = UIElement.extend({
    ...
    googleMapsInitTimeout:5000, //Stop xaiting for google maps after x seconds
    ...
    */
    ...
    gmaps_enhance2D:function(){
      ...
      if (typeof google==='undefined'){
        ...
        var self=this;
        if (self.mapOptions.providerKey){
          this.googleMapsScript += '&key=' + encodeURIComponent(self.mapOptions.providerKey);
        }
        ...
        window.gmapsCallback = callback;
        setTimeout(function(){
          if (!self.map){
            //
            self.trigger('maperror');
          }
        }, self.googleMapsInitTimeout);
      ...
      var self=this;
      var center = this.getCoords({lat:this.latitude,lng: this.longitude});
    ...
    enhance3D:function(){
      ...
      var self = this;
      //3D 
      self.map = new window.WebGLEarth('map',
               _.extend({ position:[
      ...
      self.map.setBaseMap(
        ...
        self.map.initMap(window.WebGLEarth.Maps[self.options.map3DOptions.provider.toUpperCase()], [self.options.map3DOptions.mapType,self.options.map3DOptions.providerKey])
      );
    ...
    **/
    panTo:function(location){
      var self=this;
      if (!location || !self.map || typeof location.lat=='undefined' || typeof location.lng=='undefined'){
        return false;
      }
      ...
      */
      addMarker:function(location){
        ...
        var marker;
        if (self.webGLEnabled){
          marker=self.addMarker3D(self.map, latLng);
        }
        else{
          marker=self.addMarker2D(self.map,latLng);
        }
        
        self.markers.push(marker);
        return marker;
      },
      addMarker2D:function(map,latLng){
        var marker =  new google.maps.Marker({position:latLng});
        marker.setMap(map);
        return marker;
      },
      addMarker3D:function(map,latLng){
    ...
    ¨*/
      ...
      * @return {bool} 
      **/
      isInBounds2D:function(location){
        var self = this;
        var latLng = self.getCoords(location);
        var mapBounds = self.map && self.map.getBounds();
        if (!mapBounds){
          return false;
        }
      ...
      **/
      ...
      moveToLocation2D:function(location, options, callback){
        ...
        if (!self.isInBounds(location)){
          ...
          zoomOutLoop= setInterval(function(){
            //console.log('zoom out', self.getZoom())
            if (self.isInBounds(location) || self.getZoom()<=options.minZoom){
              //console.log('stop zooming out', self.getZoom(), options.minZoom)
              clearInterval(zoomOutLoop);
              self.panTo(location);
              var zoomInLoop = setInterval(function(){
                //console.log('zoom in', self.getZoom(), initialZoom)
                if (self.getZoom()>=initialZoom){
                  //console.log('stop zooming in', self.getZoom());
                   if (callback){
...
//             console.log("Distance = " + dist.toString() + " duration " + duration);
      ...
      **/
      addOSMInGmapsSupport:function addOSMInGmapsSupport(){
         this.map.mapTypes.set("default", new google.maps.ImageMapType({
        ...
        this.map.mapTypes.set("toner", new google.maps.ImageMapType({
          getTileUrl: function(coord, zoom) {
        ...
        /* Watercolor : SLOW */
        this.map.mapTypes.set("watercolor", new google.maps.ImageMapType({
          getTileUrl: function(coord, zoom) {
