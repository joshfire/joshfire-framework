...
define([
...
], function (UIElement, UIItem, $, getid, woodman, _) {
  ...
  var DynamicContainer = UIElement.extend({
    ...
    setModel: function(model, render) {
      ...
      if (model) {
        logger.log(this.logid, 'set model', 'render=' + !!render);
     ...
     *
     * The function binds the update mechanism to all changes made to the
     * collection, meaning the view will adjust itself and may render itself
     * again whenever the collection changes.
     *
     * The function does not force an initial rendering of the view unless
    ...
    setCollection: function(collection, render) {
      ...
      if (collection) {
        logger.log(this.logid, 'set collection', 'render=' + !!render);
