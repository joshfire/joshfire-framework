  ...
  var SlidePanel = UICardPanel.extend({
    ...
    showChild: function(name, transitionDescriptor) {
      this.runTransition(_.bind(function () {
        ...
        this.transition(this.currentChild, name, transitionDescriptor);
      }, this));
    },
     ...
     *
     * Triggers a "transitionend" event when the transition is over
     *
     * @function
    ...
    /**
     * Transitions to the destination child without any transition effect.
     *
     * @function
     * @param {View} fromChild The name of the view to transition from
     *  (should already be displayed)
     * @param {View} toChild The name of the view to transition to
     */
    noTransition: function(fromChild, toChild) {
      var to = this.children[toChild || ''];
      ...
      // Position target child on top/left corner
      this.$(to.el).css({
        position: 'absolute',
        width: '100%',
        height: '100%',
    ...
    transition: function(fromChild, toChild) {
      ...
      if (!to) {
        ...
        this.noTransition(null, toChild);
      }
      else if (this.transitionType === 'slide') {
        this.slideTransition(fromChild, toChild);
      } else if (this.transitionType === 'fade') {
        this.fadeTransition(fromChild, toChild);
      }
      else {
        this.noTransition(fromChild, toChild);
      }
    },
