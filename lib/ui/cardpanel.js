  ...
  var CardPanel = UILayout.extend({
    ...
    addChild: function(name, view) {
      ...
      view.generate(_.bind(function (err, innerHTML) {
        ...
        outerHTML = outerHTML.replace(/<([^\/][^ >\/]+)/, '<$1 style="display: none;" data-joshfire-child="' + name + '"');
    ...
    removeChild: function(name) {
      if(this.children[name]) {
        this.children[name].el.innerHTML = '';

        if(this.children[name].el.parentNode) {
          this.children[name].el.parentNode.removeChild(this.children[name].el);
        }
        this.children[name].el = null;

        if(this.children[name].$el) {
          this.children[name].$el.remove();
          this.children[name].$el.off();
          this.children[name].$el = null;
        }
    ...
    setChildrenOffset: function() {
      var els = this.el.getElementsByClassName('joshfire-wrapper')[0].childNodes,
          childrenNames = _.keys(this.children);
