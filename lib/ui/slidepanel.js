...
define(["joshlib!ui/cardpanel","joshlib!vendor/underscore","joshlib!utils/dollar"], function(UICardPanel,_,$) {
  ...
  var SlidePanel = UICardPanel.extend({
    ...
    transition: function(fromChild, toChild) {
      ...
      if (from) from.trigger('hide:transitionstart');
      ...
      setTimeout(_.bind(function() {
        ...
        function transitionEndFunc () {
          ...
          if ((typeof from !== 'undefined') &&
            ...
            from.hide();
            from.trigger('hide:transitionend');
          }
          to.trigger('show:transitionend');
        }

        if (transitionEndEvent) {
          this.$('.joshfire-wrapper')
              .first()
              .one(transitionEndEvent, transitionEndFunc);
        }
        else {
          // TODO figure out what duration we should put here
          setTimeout(transitionEndFunc, 500);
        }
