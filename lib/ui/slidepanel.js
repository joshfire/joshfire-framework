...
define(["joshlib!ui/cardpanel","joshlib!vendor/underscore","joshlib!utils/dollar"], function(UICardPanel,_,$) {
  ...
  var SlidePanel = UICardPanel.extend({
    ...
    transition: function(fromChild, toChild) {
      ...
      var top = 0;
      var left = 0;
      var $surface = this.$('.joshfire-surface').first();

      if(from) {
        left = parseInt(from.$el.css('left'), 10) || left;
        top = parseInt(from.$el.css('top'), 10) || top;
      }
      else {
        this.translateX = 0;
        this.translateY = 0;
      ...
      // target child and decide on how to affect the surface's translation
      var style = {
        position: 'absolute',
        width: '100%',
        height: '100%'
    ...
    enhance: function() {
      ...
      this.$('.joshfire-viewport').first().css({
        position: 'relative',
        height: '100%',
        overflow: 'hidden'
      });
      ...
      this.$('.joshfire-surface').first().css({
        position: 'relative',
        height: '100%',
        overflow: 'visible'
      });
