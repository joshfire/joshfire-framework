...
define(['joshlib!uielement','joshlib!vendor/underscore'], function(UIElement, _) {
  ...
  var Layout = UIElement.extend({
      ...
      _.each(this.children, function (child, name) {
        child.generate(_.bind(function (err, innerHTML) {
          ...
          if(++accumulator === this.numChildren) {
            childrenOuterHTML = '<div class="joshfire-outer"><div class="joshfire-inner">' + childrenOuterHTML + '</div></div>';
    ...
    setChildrenElements: function () {
      _.each(this.children, function(child, name) {
        var el = this.el.getElementsByClassName('joshfire-inner')[0].childNodes[this.childrenOffsets[name]];
