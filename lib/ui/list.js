...
define([
  ...
  var UIList = UIElement.extend({
    initialize:function(options) {
      ...
      this.ChildrenToAdd = 0;
      this.loadMore = options.loadMore || function() {
        if (this.collection && this.ChildrenToAdd === 0) {
          console.log(this.collection.limit);
          this.ChildrenToAdd = this.collection.limit;
      ...
      if (this.autoLoadMore) {
        this.loadMoreDistance = 300;
        options.onScroll = options.onScroll || function(e, scrollHeight, scrollTop, scrollLeft) {
          if (scrollHeight - self.loadMoreDistance <= scrollTop + $(this).innerHeight()) {
            self.loadMore();
          }
        };
      } else {
    ...
    **/
    ...
    renderChildren: _.debounce(function() {
      var list = this.$el.find('*[data-joshfire-child]');
      lastOffset = list.last().data('joshfire-child');
      _(this.newChildren).each(_.bind(function(item) {
        this.generateItem(_.bind(function(err, html) {
          list.first().siblings().andSelf().last().after(html);
          --this.ChildrenToAdd;
