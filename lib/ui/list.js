  ...
  var UIList = UIElement.extend({
    ...
    initialize: function (options) {
      ...
      this.logid = this.logid || options.logid ||
        this.id || options.id ||
        '' + (options.name ? options.name : 'view') + '-' + (logid++);
      ...
      // Note the collection should trigger "load:first", "load:more" and
      // "load:last" events for the autoLoadMore feature to work properly
     ...
     *
     * The function updates and possibly flushes the view if so requested
     * once the collection is set.
     *
     * @function
     * @param {Collection} collection The Backbone collection to set
     * @param {boolean} render Flush the view once done when set.
     */
    setCollection: function (collection, render) {
      ...
      if (this.collection) {
        this.listenTo(this.collection, 'change', this.colChangedHandler);
        this.listenTo(this.collection, 'add', this.addChild);
        this.listenTo(this.collection, 'remove', this.colChangedHandler);
        this.listenTo(this.collection, 'reset', this.colChangedHandler);
        this.listenTo(this.collection, 'syncstarted', this.syncStartedHandler);
        this.listenTo(this.collection, 'syncsuccess', this.syncSuccessHandler);
        this.listenTo(this.collection, 'syncerror', this.syncErrorHandler);
      }
    ...
    /**
     * Parse the collection associated with the view to create (or re-create)
     * the list of children views.
     *
     * @function
     * @param {boolean} render Flush the view once done or not
     */
    update: function (render) {
      logger.log(this.logid, 'update', 'render=' + !!render);
      ...
      // Force a flush of the view if so requested
      if (render) {
        this.render();
      }
    },
    ...
    syncErrorHandler: function (res) {
      logger.error(this.logid, 'fetch collection items',
        'error', res.err);
      this.isLoadingMore = false;
      if (this.dataLoadingMoreClass) {
        this.$el.removeClass(this.dataLoadingMoreClass);
