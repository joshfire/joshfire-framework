    ...
    */
    {
      ...
      *    Default: &lt;div style="display:none;" class="josh-type-&lt;%=type%&gt; josh-id-&lt;%=id%&gt; &lt;%=cls%&gt;" id="&lt;%= htmlId %&gt;" data-josh-ui-path="&lt;%= path %>"&gt;&lt;%= htmlOuter %&gt;&lt;/div&gt;
      ...
      */
      getDefaultOptions: function() {
        return {
          ...
          template: '<div style="display:none;" class="josh-type-<%= type %> josh-id-<%= id %> <%= cls %>" id="<%= htmlId %>" data-josh-ui-path="<%= path %>"><%= htmlOuter %></div>',
          autoRefresh: true,
          autoInsert: true,
      ...
      */
      __constructor: function(app, path, options) {
        ...
        self.cls = self.options.addClass || '';
      ...
      */
      hasClass: function(name) {
        ...
        var self = this;
        return (' ' + self.htmlEl.className + ' ').indexOf(' ' + name + ' ') != -1;
      },
      ...
      */
      removeClass: function(name) {
        if (!name || typeof name != 'string') return;
        var self = this;
        self.htmlEl.className = (' ' + self.htmlEl.className + ' ').split(' ' + name + ' ').join(' ').slice(1, -1);
      },
      ...
      */
      toggleClass: function(name) {
        if (!name || typeof name != 'string') return;
        var self = this;
        self[self.hasClass(name) ? 'removeClass' : 'removeClass'](name);
      },
