...
define(["joshlib!ui/list","joshlib!utils/dollar","joshlib!vendor/underscore"], function(UIList,$,_) {
  ...
  var UIGrid = UIList.extend({
    ...
    /** 
    * Activate a member of the grid, given its index
    * @param {int} num index of the item in the list
    * @return {bool} successful ? false indicates the given index was out of bounds
    **/
    activate: function(num) {
      ...
      var relativeTop = top + this.translateY - this.el.getBoundingClientRect().top;
      var relativeBottom = bottom + this.translateY - this.el.getBoundingClientRect().top;

      if(relativeTop < 0) {console.log('too low')
        this.translateY -= relativeTop;
      } else if(relativeBottom > height) {console.log('too high')
        this.translateY += height - relativeBottom;
      }

      if(this.translateY) {
        $ul.css({
          'margin-top': this.translateY + 'px'
        });
      }
