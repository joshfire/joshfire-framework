define(["joshlib!ui/list","joshlib!utils/dollar","joshlib!vendor/underscore"], function(UIList,$,_) {
  ...
  var UIVerticalList = UIList.extend({
    ...
    activate: function(num) {
      this.$('li').removeClass('nav-active');
      ...
      if (this.translate3d){
        ...
        var height = $(this.el).height();
        var translateY = 0;
        //console.log('samsugtv:verticallist', 'translateY ?', ulHeight, height);
        if(ulHeight > height) {
          translateY = -num * (ulHeight - height + $lastChild.height()) / this.items.length;
        }
        else{
        ...
        /*
        console.log("UL TOP : " + translateY);
        console.log("UL H : " + ulHeight);
        console.log("UL W : " + $(this.el).width());
        console.log("THIS H : " + height);
        */
        /* scrollbar */
        if (ulHeight > height) {
          $(this.el).find('.list-indicator').css({
