      ...
      */
      {
        ...
        */
        scrollToNextPage: function() {
          ...
          //vertical
              ...
              if (!self.data[focusedIndex]){
                //console.warn('removed index ?', focusedIndex);
                //force focusedIndex to next item if available.. else 0
                if (!self.data[++focusedIndex]){
                  focusedIndex=0;
                }
              }
             var focusedHeight = $('#' + self.getItemHtmlId(self.data[focusedIndex].id), self.htmlEl).height();

              for (var i = focusedIndex; i < self.data.length - 1; i++) {
                var item=$('#' + self.getItemHtmlId(self.data[i].id), self.htmlEl);
             ...
             //   console.warn('scroll to ', 0-item.position().top-self.options.autoScrollOffset)
        ...
        */
        scrollToPrevPage: function() {
          ...
          var focusedCoords = self.grid.getCoordinatesById(focus);
          var i = 0;
          while (!focusedCoords && i<self.data.length){
            //console.warn('no focused coords','i=', i)
            focusedCoords = self.grid.getCoordinatesById(self.data[i].id);
            i++
          }
          
          if (!focusedCoords){
            console.warn('scroll err');
            return false;
          }
          
          var focusedIndex = focusedCoords[0];
         ...
         //     console.warn('removed item ?', i);
            ...
            var item = $('#' + self.getItemHtmlId(this.data[i].id));
            var elem = item.offset()[property];
            //focus to first element out of screen
            if (elem < list) break;

          }
        //  console.warn('i', i, 'focused', focusedIndex)
          if (i != focusedIndex) {
            self.focusByIndex(i);
          }
          else{
            //scroll up a little
           // console.warn('manual', self.options, {top: 0-item.position().top+2*self.options.autoScrollOffset})
             moveObj = {top: Math.max(0,0-item.position().top+2*self.options.autoScrollOffset)};
              if (self.options.scrollAnimate) {
                self.publish('scrollStart');
                $('ul',self.htmlEl).stop().animate(moveObj, self.options.scrollAnimate, self.options.scrollEasing, function() {
                  self.publish('scrollEnd');
