  ...
  */
  return Class(Input, {
    ...
    setup: function(callback) {
      ...
      // !! If we listen to click & touchstart & tap, we get 3 enter events
      //$(window).live(hasTouchEvent() ? 'touchstart click tap MozTouchDown' : 'mousedown click', function(e) {
      $(window).live(hasTouchEvent() ? 'tap MozTouchDown' : 'mousedown click', function(e) {
        for (var target = this; target && target != document; target = target.parentNode) {
          if ($(target).attr('data-josh-ui-path')) {
            ...
            if ((!elt.hasScroller && e.type!="click") || (e.type=="click" && elt.hasScroller)) {
