  ...
  return Class(Video, {
    ...
    */
    playWithStaticUrl: function(options) {
      ...
      $('#' + self.htmlId)[0].innerHTML = '<span class="close">Close</span><video id="' + self.htmlId + '_video" src="' + options.url + '" ' + (options.noautoplay ? '' : 'autoplay="true"') + ' autobuffer preload ' + (options.image ? 'poster="' + options.image + '"' : '') + ' controls=true />';
//  $('#' + self.htmlId)[0].innerHTML = '<span class="close">Close</span><video id="' + self.htmlId + '_video" src="' + options.url + '" autobuffer preload ' + (options.image ? 'poster="' + options.image + '"' : '') + ' />';
    var video = document.getElementById(self.htmlId+'_video');
 
 /*     if (!options.noautoplay){
            //video.load();
            video.play();
            if (video.paused){
              $('#'+video.id).addClass('video-paused')
            }
    }        
*/
if (video.paused){
  $('#'+video.id).addClass('video-paused')
      ...
      $('#' + self.htmlId+'_video').bind(events, function(e) {
        $('#'+video.id).toggleClass('video-paused')
        if (video.paused){
          video.play();
        }
        else{
