  ...
  */
  return Class(Video, {
    ...
    */
    ...
    var player_html = '<object id="video"'+
                                            ' type="'+(options.mime ? options.mime : 'video/mpeg4')+'"'+
                                            (options.fullScreen || options.width ? ' width="'+(options.fullScreen ? Joshfire.deviceInfo.screenWidth : options.width)+'"' : '')+ 
                                            (options.fullScreen || options.height ? ' height="'+(options.fullScreen ? Joshfire.deviceInfo.screenHeight : options.height)+'"' : '')+
                                            ' data="'+options.url+'" />';
    $('#' + self.htmlId)[0].innerHTML =player_html;
