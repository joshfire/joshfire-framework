...
//          innerTemplate: '<object id="video"></object>',
    ...
    */
    ...
    self.daemon = setInterval(self.player.onPlayStateChange=function(){self.checkPlayState(self);}, 100);
     return true;
  },
    checkPlayState:function(self){
       //  self.mylog('onPlayStateChange ' + self.player.playState+ ', '+self.videoStatus+' ['+Math.round(self.player.playPosition/1000)+' , '+Math.round(self.player.playTime/1000)+']');
                 ...
                 case 3: // connecting
                 case 4: // buffering
                  mylog('playState '+(self.player.playState==3 ? 'connecting' : 'buffering')+' ; '+self.videoStatus);
