...
//          innerTemplate: '<object id="<%=htmlId%>_video"></object>',
    ...
    */
    playWithStaticUrl: function(options) {
     ...
     if (document.getElementById('log')){
       self.mylog= function (txt){ document.getElementById('log').innerHTML = txt+'<br />'+document.getElementById('log').innerHTML;};
     }
     else{
      self.mylog=function(txt){console.warn(txt);};
     }
     self.mylog('play static '+options.url);
     if (!options.url){
    ...
    //browser testing
      self.player = video || document.getElementById(self.htmlId + '_video');
      self.mylog('video.play:'+typeof video.play)
    ...
    checkPlayState:function(){
      var self = _app.ui.element('/home/last/player');
         self.mylog('onPlayStateChange ' + self.player.playState)
    ...
    */
    getCurrentTime: function() {
      if (this.player && this.player.playPosition) return this.player.playPosition;
      return 0;
    },
