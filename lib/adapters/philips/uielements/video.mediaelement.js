...
//          innerTemplate: '<object id="video"></object>',
    ...
    */
    ...
    //browser testing
    self.mylog('video element '+self.htmlId+'_video');
      self.mylog(typeof eval(self.htmlId+'_video'))
      self.player = eval(self.htmlId+'_video') || document.getElementById(self.htmlId + '_video');
      self.mylog('self.player.play:'+typeof self.player.play)
      self.player.data=options.url;
      self.mylog('data: '+self.player.data)
      self.mylog('check: '+eval(self.htmlId+'_video').data)
