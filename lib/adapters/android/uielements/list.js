          ...
          self.subscribe('input', function(ev, data) {
            ...
            if (data.length >= 2 && data[0] == 'enter') {
              ...
              if (self.options.multiple){
                //multiple select
                var selected = self.getState('selection') || [];
                if (!_.contains(selected, data[1])){
                  selected.push(data[1]);
                }
                else{
                  selected = _.reject(selected, function(item){return item==data[1];});
                }
                self.selectById(selected);
              }
              else{
...
//              self.selectById(data[1]);
        ...
        insertScroller: function() {
          ...
          if (self.scrollOptions.active && self.data && self.data.length && self.htmlEl.children.length) {
            ...
            self.iScroller = new iScroll(self.htmlId, _.extend({useTransition: true}, self.scrollOptions));
